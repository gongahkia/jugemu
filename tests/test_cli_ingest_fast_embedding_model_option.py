...
def test_cli_ingest_passes_fast_embedding_model(monkeypatch):
    runner = CliRunner()
    ...
    res = runner.invoke(cli.app, ["ingest", "--fast-embedding-model"])
    assert res.exit_code == 0
    assert captured.get("fast_embedding_model") is True
